from app.db.local.user_repository import create_user, get_user_by_dni, get_user_by_email
...
def register_user(data: RegisterRequest) -> RegisterResponse:
    ...
    existing_user = get_user_by_dni(data.dni)

    if existing_user:
        raise ValueError("El DNI ingresado ya pertenece a un usuario registrado")

    existing_email = get_user_by_email(str(data.email))
